...
-- Per-repo (environment='') and per-environment values. Secret values are
-- AES-256-GCM encrypted with the master key; variables are plaintext.
CREATE TABLE IF NOT EXISTS repo_secrets (
    ...
    created_at  INTEGER NOT NULL,
    updated_at  INTEGER NOT NULL,
    UNIQUE(repo_id, environment, name)
);
